...
const DELAYS = {
  shortDelay: 800,      // Increased for stability
  mediumDelay: 1500,    // Increased for stability
  longDelay: 3000       // Increased for stability
};
...
const TIMEOUTS = {
  ...
  operation: 45000      // 45s for operations to complete
};
...
const MAX_CONCURRENT_TABS = 1; // Reduced from 5 - INCREASE GRADUALLY if stable
...
async function ensureLoggedIn(page) {
  ...
  await page.waitForSelector(SELECTORS.usernameInput, { timeout: TIMEOUTS.selector });
...
async function createAccount(page, data, accountId, tabId) {
  ...
  try {
    ...
    await page.waitForSelector(SELECTORS.typeSelector, { timeout: TIMEOUTS.selector });
    ...
    // Fast form filling using evaluate with timeout protection
    await Promise.race([
      page.evaluate((data, selectors) => {
      ...
      new Promise((_, reject) => 
        setTimeout(() => reject(new Error('Form fill timeout')), TIMEOUTS.operation)
      )
    ]);
    
    await page.click(SELECTORS.next1Button);
    await page.waitForSelector(SELECTORS.companyNameInput, { 
      visible: true, 
      timeout: TIMEOUTS.selector 
    });
    await new Promise(resolve => setTimeout(resolve, DELAYS.mediumDelay));
    
    // Fast form filling for company details with timeout protection
    await Promise.race([
      page.evaluate((data, selectors) => {
    ...
    try {
      ...
      console.log(`${prefix} ⚠️  State field skipped: ${error.message}`);
    }
    
    // Handle phone with timeout
    if (data.Phone) {
      try {
        await Promise.race([
          page.evaluate((phone, selector) => {
          ...
          new Promise((_, reject) => 
            setTimeout(() => reject(new Error('Phone fill timeout')), 10000)
          )
        ]);
        await new Promise(resolve => setTimeout(resolve, DELAYS.shortDelay));
      } catch (error) {
        console.log(`${prefix} ⚠️  Phone field skipped: ${error.message}`);
      }
    }
    
    await page.click(SELECTORS.submitButton);
    await new Promise(resolve => setTimeout(resolve, DELAYS.longDelay));
